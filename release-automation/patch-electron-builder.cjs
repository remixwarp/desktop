...
LinuxTargetHelper.prototype.computeMimeTypeFiles = async function (...args) {
    ...
    const xmlPath = pathUtil.join(__dirname, '../linux-files/org.remixwarp.RemixWarp.mime.xml');
    const rawXml = await fsPromises.readFile(xmlPath, 'utf-8');
    const newXml = rawXml.replace(/org\.remixwarp\.RemixWarp/g, packageJSON.name);
